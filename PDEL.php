...
$todel=$_GET['rno'];
mysqli_query($link,"update Patient SET pshow='N' where AAdharid='$todel' ;");
$rs1=mysqli_query($link,"SELECT * from patient where pshow='Y' order by
pname;");
//$sno=1;
while( $row=mysqli_fetch_array($rs1)) {
 echo "<tr><td>$row[0]</td><td>$row[1]</td><td>$row[2]</td><td>$row[3]</td><td>$row[4]</td><td><a
...
align=center>Options</td></tr>
<?php
$rs2=mysqli_query($link,"SELECT * from patient where pshow='N' order by
pname;");
$sno=1;
